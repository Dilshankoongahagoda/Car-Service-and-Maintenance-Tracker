...
<head>
    ...
    <title>Create Invoice - Shift Auto Dynamics</title>
    ...
    <style>
        ...
        .appt-info-card {
            ...
            padding: 20px 25px; border-left: 5px solid var(--primary); margin-bottom: 25px;
        }
        ...
        /* Service items from appointment - read only */
        .readonly-services { margin-bottom: 20px; }
        .readonly-svc-item {
        ...
        /* Additional Services - Category Accordion */
...
<div class="page-container">
    <div class="page-header" style="border-bottom: 3px solid var(--primary); padding-bottom: 15px; margin-bottom: 30px;">
        <h1 style="font-family: 'Oswald', sans-serif; font-size: 2.5rem; color: var(--dark);">CREATE INVOICE</h1>
    </div>

    <div class="form-container">
        <!-- Appointment Info Card -->
        <div class="appt-info-card">
            <h4>📋 Appointment #${appointment.appointmentId} — COMPLETED</h4>
            <div class="appt-info-row">
                <div class="appt-info-item">
                    <div class="lbl">Customer</div>
                    <div class="val">${appointment.userName}</div>
                </div>
                <div class="appt-info-item">
                    <div class="lbl">Vehicle</div>
                    <div class="val">${appointment.vehicleInfo}</div>
                </div>
                <div class="appt-info-item">
                    <div class="lbl">Service Date</div>
                    <div class="val">${appointment.preferredDate} at ${appointment.preferredTime}</div>
                </div>
            </div>
        </div>
        ...
        <form action="estimate" method="post" id="invoiceForm">
            ...
            <!-- SECTION 1: Original Services from Appointment -->
            <div class="form-section">
                <h3>🔧 Booked Services (from Appointment)</h3>
                <div class="readonly-services" id="originalServicesList">
                    <c:forTokens var="svc" items="${appointment.serviceName}" delims="," varStatus="loop">
                        <div class="readonly-svc-item">
                            <span class="name">${svc}</span>
                            <span class="price" data-original-price="true">—</span>
                        </div>
                    </c:forTokens>
                </div>
                <p style="font-size: 0.8rem; color: var(--text-muted); margin: 0;">
                    Estimated price from booking: <strong style="color: var(--primary);">${appointment.servicePrice}</strong>
                </p>
            </div>

            <!-- SECTION 2: Additional Services (Select from existing services) -->
            <div class="form-section">
                <h3>➕ Additional Services Performed</h3>
                <p style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 12px;">Select any extra services that were performed beyond the original booking.</p>
            ...
            <div class="form-section">
                ...
                <p style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 12px;">Add all parts and materials used during the service.</p>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    <textarea name="notes" rows="3" placeholder="Any additional notes, warranty info, or terms..."></textarea>
                </div>
            </div>
            ...
            <div style="display: flex; gap: 15px; justify-content: flex-end; margin-top: 25px;">
                <a href="appointment?action=completed" style="padding: 12px 30px; border: 1px solid var(--border); border-radius: 6px; text-decoration: none; color: var(--dark); font-weight: 600;">CANCEL</a>
                <button type="submit" class="btn-primary-custom" style="padding: 12px 30px; font-size: 1rem; border: none; cursor: pointer; font-weight: 700;">📑 GENERATE INVOICE &gt;</button>
            </div>
        </form>
    </div>
</div>
...
<script>
// Parse price - data-price now contains clean numeric values from server
// This function handles both clean numbers and any legacy formatted strings
function parsePrice(priceStr) {
    if (!priceStr) return 0;
    var str = String(priceStr).trim();
    // Try direct parse first (for clean numeric values)
    var direct = parseFloat(str);
    if (!isNaN(direct) && str.match(/^[0-9.]+$/)) return direct;
    // Remove all currency text and symbols
    str = str.replace(/Rs\.?/gi, '').replace(/LKR/gi, '').replace(/USD/gi, '').replace(/EUR/gi, '');
...
// Parse the original appointment price
var originalPriceStr = '${appointment.servicePrice}';
var originalPriceNum = parsePrice(originalPriceStr);

// Store original service names
var originalServiceNames = [];
<c:forTokens var="svc" items="${appointment.serviceName}" delims=",">
...
function addPart() {
    ...
    row.innerHTML = '<input type="text" class="part-name" placeholder="e.g. Brake Pads (Front)" />' +
                    '<input type="number" class="part-price" placeholder="4500" min="0" step="0.01" oninput="recalcTotals()" />' +
...
function recalcTotals() {
    // Sum additional services from checkboxes
...
document.getElementById('invoiceForm').addEventListener('submit', function(e) {
    ...
    // Parse the original price to a clean number
    document.getElementById('hServicePrices').value = originalPriceNum.toFixed(2);

    // Additional services from checkboxes
    ...
    document.getElementById('hTax').value = tax.toFixed(2);
